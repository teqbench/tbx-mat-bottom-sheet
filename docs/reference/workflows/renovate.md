...
**File:** central — `teqbench/.github/.github/workflows/renovate.yml` and `teqbench/.github/renovate-config.js`. There is no per-repo [Renovate ↗](https://docs.renovatebot.com/) config in this repository.
...
The workflow lives in `teqbench/.github/.github/workflows/renovate.yml` and runs every three hours under the `teqbench-automation` GitHub App. It reads `teqbench/.github/renovate-config.js`, which lists the repositories [Renovate ↗](https://docs.renovatebot.com/) should manage. New repositories must be added to that list to be enrolled.
...
All [Renovate ↗](https://docs.renovatebot.com/) PRs target **`dev`** (`baseBranchPatterns: ["dev"]`). Updates flow through the standard PR review and CI pipeline before reaching production.
...
```yaml
uses: teqbench/.github/.github/workflows/ci.yml@<full-sha> # main
```
...
Internal `@teqbench/*` packages are configured with `automerge: true` and `automergeType: "pr"`. When CI passes, [Renovate ↗](https://docs.renovatebot.com/) merges the PR automatically — no human review required for internal version bumps.
...
<dl>
    ...
    <dd>Restriction: <code>&lt; 10.0.0</code>. <a href="https://eslint.org">ESLint ↗</a> majors are held until <a href="https://github.com/angular-eslint/angular-eslint">angular-eslint ↗</a> supports them across the framework's package family.</dd>
    <dt><code>@types/node</code></dt>
    <dd>Restriction: <code>&lt; 25.0.0</code>. Pinned to the major matching the <a href="https://nodejs.org/">Node.js ↗</a> runtime (currently 24). Re-evaluated on each Node LTS bump.</dd>
</dl>
...
To stop [Renovate ↗](https://docs.renovatebot.com/) from managing this repository, remove it from `repositories[]` in `teqbench/.github/renovate-config.js`. To pause individual packages, add a `packageRules` entry with `enabled: false` matched against the package name.
