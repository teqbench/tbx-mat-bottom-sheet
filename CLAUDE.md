...
All public exports follow the `@teqbench` naming convention. The PascalCase / UPPER_SNAKE_CASE prefix is determined by the package scope:
...
Whenever a `package.json` field intentionally constrains a dependency or build behavior in a way that is non-obvious — a non-default version range, a hold-back, an [npm ↗](https://www.npmjs.com) `overrides` entry — record the rationale in a sibling `<field>Pinned` field rather than a free-form comment. This makes the intent machine-readable, lints cleanly, and survives format-on-save tooling that strips comments.

### Shape

The value is an object with a single `"//"` key containing an array of rationale lines:

```json
{
    "devDependenciesPinned": {
        "//": [
            "typescript-eslint is pinned without ^ because patch releases have historically",
            "introduced breaking rule changes. Pin to a known-good version and update deliberately."
        ]
    }
}
```
...
- One `<field>Pinned` per constrained `package.json` field. Examples: `devDependenciesPinned`, `dependenciesPinned`, `peerDependenciesPinned`, `overridesPinned`, `enginesPinned`.
- Each rationale entry names the package (or constraint key) and explains _why_ the pin exists and _when_ it can be lifted. A reader should be able to remove the pin without reading external context.
- Do not use bare `"//"` keys at the top level of `package.json` for the same purpose — the `<field>Pinned` shape disambiguates which field the rationale applies to.
- Custom-field rule from the AI Friendliness Convention applies: the field must be identified as custom wherever it is referenced in prose (it is not defined by the [npm ↗](https://www.npmjs.com) spec).

### Verification

[npm ↗](https://www.npmjs.com) ignores unknown top-level fields. Custom `<field>Pinned` keys do not affect install, publish, or lockfile behavior.
